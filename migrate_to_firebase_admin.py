#!/usr/bin/env python3
"""
Data Migration Script: CSV to Firebase Firestore (Admin SDK)
Migrates missing persons data from CSV to Firebase Firestore using Admin SDK.
...
SERVICE_ACCOUNT_PATH = "firebase-service-account.json"  # Download from Firebase Console
COLLECTION_NAME = "missing_persons"
BATCH_SIZE = 500  # Firestore batch write limit

class FirebaseAdminMigrator:
    def __init__(self):
        self.db = None
    ...
    def check_existing_data(self) -> int:
        ...
        try:
            ...
            # Get approximate count (Firestore doesn't have a direct count)
            # For now, we'll just check if any documents exist
            return len(docs)
        except Exception as e:
...
def main():
    """Main entry point."""
    print("Firebase Firestore Migration Tool (Admin SDK)")
    print("=" * 45)
    
    migrator = FirebaseAdminMigrator()
    ...
    if success:
        ...
        print("3. Update your frontend to use the new API endpoints")
    else:
